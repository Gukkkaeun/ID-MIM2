...
class KT_Boat(BaseImageDataset):
    ...
    """
    # 核心配置抽离，便于维护和扩展
    DATASET_NAME = 'KT_Boat'
    MODALITIES = ['VIS', 'IR']  # 模态名称与目录名严格对应
    MOD2CAMID = {'VIS': 0, 'IR': 1}  # 模态映射为camid（兼容ReID框架）
    ...
    def _load_ids(self, file_path):
        ...
        try:
            formatted_ids = [f"{int(pid_str):04d}" for pid_str in ids_str_list]
    ...
    def _process_dir_train(self, relabel=False):
        ...
        # 4. 构建VIS-IR成对数据集（以VIS为基准）
        pair_dataset = []
        vis_pid_paths = mod2pid2paths['VIS']
        for pid in vis_pid_paths:
            ...
            for vis_path in vis_pid_paths[pid]:
                ...
                for ir_path in mod2pid2paths['IR'][pid]:
                    ...
                    ir_item = (ir_path, self.pid_begin + ir_pid_label, self.MOD2CAMID['IR'], 1)
                    pair_dataset.append([vis_item, ir_item])
    ...
    def _print_dataset_stats(self, train, val, test, train_pair):
        ...
        """
        print("=" * 60)
        print(f"=> KT_Boat ReID Dataset 加载完成")
        print("-" * 60)
        # 基础数据集统计
