...
class CMship(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, pid_begin=0, **kwargs):
        ...
        if verbose:
            ...
            if train_pair is not None:
                print("Number of VIS-NIR/TIR pair: {}".format(len(train_pair)))
                print("  ----------------------------------------")
    ...
    def _process_dir(self, mode, relabel=False):
        ...
        for img_path in img_paths:
            ...
            pid_str = img_path.split('\\')[-2]
            pid = int(pid_str)
            mod = img_path.split('\\')[-3]  # VIS/NIR/TIR
            ...
            for vis_path in vis_paths[pid]:
                ...
                if pid in mod2pid2paths['NIR']:
                    for nir_path in mod2pid2paths['NIR'][pid]:
                        ...
                        nir_camid = self.mod2camid['NIR']
                        nir_item = (nir_path, self.pid_begin + nir_pid_label, nir_camid, 1)
                ...
                if pid in mod2pid2paths['TIR']:
                    for tir_path in mod2pid2paths['TIR'][pid]:
                        ...
                        tir_camid = self.mod2camid['TIR']
                        tir_item = (tir_path, self.pid_begin + tir_pid_label, tir_camid, 1)
